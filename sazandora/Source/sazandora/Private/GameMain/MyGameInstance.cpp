...
void UMyGameInstance::Shutdown()
{
	...
	// 起動中のサーバープロセスをすべて終了し、ハンドルを解放する
	for (FProcHandle& Handle : ServerProcessHandle)
	{
		if (Handle.IsValid())
		{
...
void UMyGameInstance::Set_ServerProcess(FProcHandle handle)
{
	ServerProcessHandle.Add(handle);
	UE_LOG(LogTemp, Warning, TEXT("Add_ProcessHandle!!"));
}